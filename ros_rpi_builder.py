...
ROS_DEFAULT_PACKAGE = "ros-galactic-ros-base"
...
class EnvironmentManager:
    ...
    def download_ros_key(self):
        if not (path.exists(EnvironmentManager.LOCAL_GPG)):
            process = subprocess.run([
                'wget', self.parsed_args.gpg_key_url, '-O',
                EnvironmentManager.LOCAL_GPG
            ],
                                     check=True,
    ...
    def create_sources_list(self, name, type, distro):
        url = "{type} [signed-by={gpg}] {repo_url} {distro} {component}\n".format(
            type=type,
            gpg=EnvironmentManager.LOCAL_GPG,
            repo_url=self.parsed_args.repo_url,
            distro=distro,
            component=self.parsed_args.component)
    ...
    def create_ros_deb_src(self):
        self.create_sources_list('ros2_src.list', 'deb-src',
                                 self.parsed_args.src_distro)
    ...
    def run_apt_get(self, command, packages=None):
        ...
        if packages == None:
            ...
            process = subprocess.run(['apt-get', '--yes', command] + packages,
                                     **apt_args)
        return process
    ...
    def download_misc_debs(self):
        if not (path.exists(EnvironmentManager.LOCAL_REPO_DIR +
                            '/rti-connext-dds-5.3.1_0.0.0-0_arm64.deb')):
            subprocess.run([
                'wget',
                'http://packages.ros.org/ros2/ubuntu/pool/main/r/rti-connext-dds-5.3.1/rti-connext-dds-5.3.1_0.0.0-0_arm64.deb'
            ],
                           check=True,
                           cwd=EnvironmentManager.LOCAL_REPO_DIR,
                           stdout=subprocess.DEVNULL,
...
class RosModule:
    ...
    def install_build_deps(self):
        ...
        envManager.run_apt_get('build-dep', [self.name])
    ...
    def update_deb_changelog(self):
        patches = {envManager.parsed_args.src_distro: envManager.release}
    ...
    def move_deb_pkg(self):
        ...
        if path.exists(deb_location):
            shutil.move(deb_location, EnvironmentManager.LOCAL_REPO_DIR)
        else:
    ...
    def prepare_install_deps(self):
        ...
        if not self.can_install_ros_module():
            raise RuntimeError(
                'Unsatisified install dependency for module {}'.format(
                    self.name))
    ...
    def release_name(self):
        reg = r"ros-([a-z]+)-"
        matches = re.search(reg, self.name)
        if matches:
            return matches.group(1)
        else:
            raise RuntimeError("Unable to find release")
...
def handle_args():
    ...
    ap.add_argument('--package',
                    required=False,
                    default=ROS_DEFAULT_PACKAGE,
                    help='package to install')
    ap.add_argument('--src-distro',
                    required=False,
                    default='focal',
                    help='debian distro name')
    ap.add_argument('--component',
                    required=False,
                    default='main',
                    help='debian components e.g. main')
    ap.add_argument('--repo-url',
                    required=False,
                    default='http://packages.ros.org/ros2/ubuntu',
                    help='url of ros repo')
    ap.add_argument(
        '--gpg-key-url',
        required=False,
        default=
...
def build_and_install(parsed_args):
    ...
    requested_ros_module = envManager.find_or_create_module(
        parsed_args.package)

    # Build requested module
    requested_ros_module.apt_install_ros_module()
